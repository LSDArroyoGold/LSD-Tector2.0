...
if AUTO_SYNC == 'ON':
	duracion_h = '$(awk -F'=' '/DURACION_ATARDECER_SYNC/{print $2}' "$CONFIG_HORARIOS" | tr -d ' \r')'
else:
	from datetime import datetime
	t_inicio = datetime.strptime('$(awk -F'=' '/INICIO_ATARDECER/{print $2}' "$CONFIG_HORARIOS" | tr -d ' \r')', '%H:%M')
	t_fin = datetime.strptime('$(awk -F'=' '/FIN_ATARDECER/{print $2}' "$CONFIG_HORARIOS" | tr -d ' \r')', '%H:%M')
...
")
	...
	if [ "$NIVEL" -lt "$UMBRAL" ]; then
		...
		python3 "$BASE_PATH/python/log_sistema.py" CANCELADA atardecer
